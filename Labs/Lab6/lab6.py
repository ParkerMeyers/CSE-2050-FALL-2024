class OrderedList:
    ...
    def _bs(self, item, left, right, median=None):
        """
        Searches for item using `left` and `right` indices instead of slicing
        Parameters:
            item: the item to search for
            left: the left index of the search
            right: the right index of the search
            median: the median index
        Returns:
        ...
        # Calculate the median index if it's not provided
        if median is None: median = (left + right) // 2
